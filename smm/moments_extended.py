...
def get_moments_extended (data_frame):
    ...
    data_frame['Full Time Ind'] = data_frame.apply (lambda row: get_full_time_ind (row),axis=1)
    ...
    info_test1 = data_frame.groupby(['Period', 'Educ Level'])['Wage Observed'].describe().to_dict()
    ...
    for period in sorted(data_frame['Period'].unique().tolist()):
        for educ_level in sorted(data_frame['Educ Level'].unique().tolist()):
            ...
            for label in ['mean', 'std']:
                moments['Wage by Educ'][(period, educ_level)].append(info_test1[label][(period, educ_level)])
    ...
    info_test4 = data_frame.groupby(['Period', 'Educ Level'])['Choice'].value_counts(normalize=True).to_dict()
    ...
    info_test5 = data_frame.groupby(['PT CS'])['Choice'].value_counts(normalize=True).to_dict()
    ...
    info_test6 = data_frame.groupby(['FT CS'])['Choice'].value_counts(normalize=True).to_dict()
